...
- CSS for the checkbox group (`.wao-checkbox-group`, `.wao-checkbox-item`, `.wao-checkbox`, `.wao-checkbox-text`): custom-drawn 18px checkboxes on the shared border/radius/shadow tokens, focus ring, hover-highlighted rows, inline SVG checkmark on the focus color, and suppression of WP core's own `::before` checkmark on admin checkboxes.
- "Select (Checkboxes)" demo on the test fields page.
- `format => 'checkboxes'` on `TaxonomySelectOption` for multiple mode, the same presentation as `SelectOption`'s: one checkbox per term (the "Select ..." placeholder is skipped), posting checked term ids natively as `key[]` after a hidden sentinel input named `key`, so an all-unchecked state still posts the key (as an empty string) and the stored terms can be cleared. Uses the existing `.wao-checkbox-*` CSS. "Taxonomy Select (Checkboxes)" demo on the test fields page. `src/version.php` 1.2.0.
